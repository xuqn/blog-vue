		...
		<section>
			<div class="item" v-for="blog in blogs" :key="blog.key">
				<figure @click="onGetuser(blog.user.id)">
					<img :src="blog.user?blog.user.avatar:''" :alt="blog.user?blog.user.username:''">
					<figcaption>{{blog.user?blog.user.username:''}}</figcaption>
				</figure>
...
export default {
  ...
  data() {
	...
	created() {
		blog.getIndexBlogs({ page: this.$route.query.page })
		.then(res => {
			this.blogs = res.data
